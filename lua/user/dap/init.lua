...
if not (ok_dap and ok_ui) then
  if not ok_dap then vim.notify("nvim-dap not available", vim.log.levels.ERROR) end
  if not ok_ui  then vim.notify("nvim-dap-ui not available", vim.log.levels.ERROR) end
  return
end
